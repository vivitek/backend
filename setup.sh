...
cat utils/webhook-handler.model.sh | sed "s~{{EXEC_PATH}}~$PWD~g" > ./webhook-handler.sh
chmod 777 ./webhook-handler.sh

# Add restart-docker service
cat utils/restart-docker.service | sed "s~{{EXEC_PATH}}~$PWD~g" > /tmp/restartDockerService
