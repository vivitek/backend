...
const {checkPermission} = require("../middleware/ban.middleware");

router.get("/", checkTokenValidity, checkPermission, async(req, res) => {
	var list = await banModel.find();
	res.json(list);
});

router.get("/:id", checkTokenValidity, checkPermission, async(req, res) => {
	let {id} = req.params;
	let ban;
	try {
...
router.delete("/:id", checkTokenValidity, checkPermission, async(req, res) => {
	let {id} = req.params;
	let resu = await banModel.findByIdAndDelete(id);
	res.json(resu);
});

router.post("/", checkTokenValidity, checkPermission, async(req, res) => {
	var newBan = await banModel.create(req.body);
	res.status(201).json(newBan);
});

router.patch("/:id", checkTokenValidity, checkPermission, async(req, res) => {
