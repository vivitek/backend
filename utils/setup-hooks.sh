...
if [ $? = 1 ]; then
    echo "\033[31m\033[1mESlint is required. Please install ESlint.\033[0m"
    exit 1
fi
...
for file in $STAGED_FILES; do
    ...
    if [ $output != "" ]; then
        ...
        echo "[ \033[32m\033[1mOK\033[0m ] $file"
    fi
done

# Final output
if ! $VALID; then
    echo "\033[31m\033[1mCommit failed, your code contains some errors, try --fix.\033[0m"
    exit 1
fi

echo "\033[32m\033[1mCommit succeed.\033[0m"' > .git/hooks/pre-commit
...
echo $current_branch | grep -qP "^(feature|improvement|hotfix|fix)\/[a-zA-Z0-9-_]+" > /dev/null
if [ $? = 1 ]; then
    echo "\033[31m\033[1mYour branch name is valid.\nPatern : (feature|improvement|fix)/branchName\033[0m"
    exit 1
